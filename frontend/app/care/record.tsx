...
// Date.toISOString()은 UTC 기준 숫자를 뽑는다. 이 화면은 그 문자열을 그대로 입력칸에 보여주고
// 손대지 않으면 new Date(text)로 다시 파싱하는데, 오프셋 없는 문자열은 로컬 시간대로 해석되므로
// "UTC 숫자를 로컬로 착각해서 되읽는" 이중 오차가 생긴다 — KST(UTC+9)면 9시간 이르게 저장돼
// 같은 날 먼저 만들어진 다른 기록보다 타임라인에서 뒤로 밀린다. 로컬 필드로 직접 조합해 피한다.
