...
public class TimelineService {
    ...
    public TimelineEventResponse createEvent(Long passportId, Long requesterAccountId,
        ...
        try {
            ...
            // try 안에 둬야 여기서 나는 예외도 아래 catch의 고아 이미지 정리를 탄다.
            // 잠금을 안 거는 이유는 DiagnosisService.submit()의 같은 지점 주석 참고.
    ...
    // 6종 소스를 전부 읽어 메모리에서 합쳐 정렬한다. 페이지네이션은 아직 없다 — 타입별 LIMIT로는
    // 전역 정렬 순서를 보장 못 해서 API 계약을 바꿔야 붙일 수 있다.
    public List<TimelineItem> getTimeline(
        ...
        // CANCELLED로 바뀐 예약도 상태값 그대로 노출한다 — 다른 소스가 이력을 숨기지 않는 것과
        // 같은 원칙(스펙 문서 6절 참고).
        List<Reservation> reservations =
            reservationRepository.findAllByPassportId(passportId);
        // 예약 건마다 findById를 부르면 N+1이 된다 — storeId들을 한 번에 배치 조회한다.
        // ReservationService와 로직이 같아서 StoreRepository.namesByIds()로 공유한다.
        ...
        // 승계(소유권 이전)도 여권 일생의 주요 이벤트라 예약과 같은 방식으로 소스에 추가한다.
        // 여권 하나가 여러 번 승계될 수 있으므로 완료(REDEEMED)된 코드를 전부 가져온다.
    ...
    public void deleteEvent(Long eventId, Long requesterAccountId) {
        ...
        // 이벤트 행만 지우고 이미지를 그대로 두면 Cloudinary에 고아 파일이 쌓인다.
        // 이미지 정리 실패로 삭제 자체가 막히면 안 되므로 베스트에포트로 처리한다.
