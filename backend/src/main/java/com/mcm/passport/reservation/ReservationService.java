...
public class ReservationService {
    ...
    public ReservationResponse create(Long passportId, Long requesterAccountId, CreateReservationRequest request) {
        // 잠금 없는 조회를 쓰면, 이 예약 INSERT가 AccountService.withdraw()/PassportService.delete()의
        // cancelAllRequestedForPassport() 벌크 취소가 같은 여권에 대해 이미 실행된 뒤에 커밋될 수 있다
        // — 그러면 이 예약은 REQUESTED로 영원히 남아 그 매장·시각 슬롯을 아무도 못 잡게 된다
        // (PassportService.delete()가 이 클래스와 동일한 이유로 겪은 경합).
    ...
    public Page<ReservationResponse> list(Long passportId, Long requesterAccountId, Pageable pageable) {
        ...
        // slotDateTime만으로는 동시각 예약 두 건 사이에 타이브레이커가 없어 페이지 간 순서가
        // 불안정해질 수 있다 — PassportService.list()와 같은 방식으로 id를 덧붙인다
        // . 리포지토리 메서드 이름의 OrderBy(slotDateTime desc)가 먼저 적용되고
        // 그 뒤에 이 Pageable의 정렬이 이어붙는다.
