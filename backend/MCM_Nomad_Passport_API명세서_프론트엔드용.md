...
기준 커밋: `9cea33a` (fix: cancel pending reservations when an account withdraws)
...
| SLOT_ALREADY_BOOKED | 409 | 이미 예약된 시간 (동시 요청/중복 예약) |
| INVALID_SLOT_TIME | 400 | 매장 영업시간·슬롯 그리드에 맞지 않는 예약 시간 |
...
| RESERVATION | reservationId | `{ storeName, requestItems, status }` (`status`가 `CANCELLED`인 것도 포함됨, 필터링 안 함) |

> `RESERVATION` 항목의 `occurredAt`은 예약 생성 시각이 아니라 **예약된 슬롯 시각**(`slotDateTime`)임 — 다른 타입들과 기준이 다르니 주의.
...
## 8. 매장 및 정식 케어 예약 (Store & Reservation) — `/api/stores`, `/api/passports/{passportId}/reservations`, `/api/reservations`

> 매장(Store)은 읽기 전용 시드 데이터(3개 매장, DB 마이그레이션으로만 채워짐) — 매장 CRUD API는 없음(기획 확정 사항).

### GET `/api/stores` (인증 필요)
매장 목록 (페이지네이션) → `200 OK` + `Page<StoreResponse>`
```json
{
  "id": 1, "name": "MCM 강남점", "address": "서울 강남구 압구정로 165",
  "businessHoursStart": "10:00:00", "businessHoursEnd": "19:00:00", "slotLengthMinutes": 60
}
```

### GET `/api/stores/{id}` (인증 필요)
매장 상세 → `200 OK` + `StoreResponse` (위와 동일 형태)
실패 시 `404 STORE_NOT_FOUND`

### GET `/api/stores/{storeId}/available-slots?date=` (인증 필요)
해당 매장·해당 날짜의 예약 가능 슬롯 시각 목록. 영업시간을 `slotLengthMinutes` 단위로 나눈 그리드에서 이미 `REQUESTED` 상태로 예약된 시각과 현재 시각(서버 기준) 이전 시각을 제외하고 반환. 그리드는 슬롯 길이로 딱 나누어떨어지지 않는 마지막 부분 슬롯(trailing partial slot)은 포함하지 않음. 자정을 넘어가는 영업시간(예: 22:00~02:00)은 지원하지 않음.

쿼리 파라미터: `date` — `YYYY-MM-DD` (ISO date)

응답: `200 OK` — **페이지네이션 없음**, 배열 그대로 반환
```json
["2026-09-01T10:00:00", "2026-09-01T11:00:00", "2026-09-01T13:00:00"]
```
실패 시 `404 STORE_NOT_FOUND`

### POST `/api/passports/{passportId}/reservations` (인증 필요)
정식 케어 예약 생성 — passport 소유자 본인만 가능(다른 규격들과 동일하게 `PassportOwnershipGuard`로 소유권/활성 상태 확인)

요청 본문:
```json
{
  "storeId": 1,                              // @NotNull
  "slotDateTime": "2026-09-01T14:00:00",     // @NotNull @Future
  "requestItems": ["LEATHER_CLEANING"]        // @NotNull, 최소 1개, 아래 enum 참고
}
```
응답: `201 CREATED`
```json
{
  "id": 1, "passportId": 1, "storeId": 1, "storeName": "MCM 강남점",
  "slotDateTime": "2026-09-01T14:00:00",
  "requestItems": ["LEATHER_CLEANING"], "status": "REQUESTED",
  "createdAt": "2026-08-13T12:00:00"
}
```
실패:
- `404 STORE_NOT_FOUND` — 없는 매장 ID
- `400 INVALID_SLOT_TIME` — 매장 영업시간·슬롯 그리드에 맞지 않는 시각(예: 10분 단위로 어긋난 시각, 영업시간 밖)
- `409 SLOT_ALREADY_BOOKED` — 같은 매장·같은 시각이 이미 `REQUESTED` 상태로 예약됨. **여권 단위가 아니라 매장+시각 단위로 정원을 관리** — 같은 계정이 소유한 다른 여권으로도 동일 슬롯 재요청 시 막힘.

### GET `/api/passports/{passportId}/reservations` (인증 필요)
해당 여권의 예약 이력 (페이지네이션, `slotDateTime` 내림차순) → `200 OK` + `Page<ReservationResponse>` (위 생성 응답과 동일 형태)

### GET `/api/reservations/{id}` (인증 필요)
예약 상세 → `200 OK` + `ReservationResponse`
실패 시 `404 RESERVATION_NOT_FOUND` (예약 ID 자체가 없음) 또는 `403 FORBIDDEN` (예약은 존재하지만 그 예약이 속한 여권이 본인 소유가 아님 — 소유권 검증 단계에서 걸림) 또는 `404 PASSPORT_NOT_FOUND` (예약이 속한 여권이 삭제됨 — 여권 삭제 시 예약은 자동 취소되지만 예약 행 자체는 남아있음)

### PATCH `/api/reservations/{id}/cancel` (인증 필요)
예약 취소 → `204 NO_CONTENT`
**멱등** — 이미 `CANCELLED`인 예약에 다시 호출해도 에러 없이 `204` (상태를 미리 확인하지 않고 그대로 `CANCELLED`로 재기록).
실패 시 `404 RESERVATION_NOT_FOUND` (예약 ID 자체가 없음) 또는 `403 FORBIDDEN` (예약은 존재하지만 그 예약이 속한 여권이 본인 소유가 아님) 또는 `404 PASSPORT_NOT_FOUND` (예약이 속한 여권이 삭제됨 — 여권 삭제 시 예약은 자동 취소되지만 예약 행 자체는 남아있음) — `GET /api/reservations/{id}`와 동일한 소유권 검증 경로(`getOwnedReservation`)를 거침.

**Enum**
- `CareRequestItemType`: `LEATHER_CLEANING`, `METAL_POLISHING`, `STITCHING_REPAIR`, `OTHER`
- `ReservationStatus`: `REQUESTED`, `CANCELLED` (`COMPLETED` 등 다른 상태 없음 — 기획 확정 사항, §10 YAGNI)

> ⚠️ 알려진 미해결 레이스: 취소 직후 같은 슬롯을 즉시 재예약하는 아주 좁은 타이밍 창에서 경합이 날 수 있음 — 스펙 문서에서 의도적으로 남겨둔 것으로 확인됨(테스트/수정 대상 아님).
...
| CareRequestItemType | LEATHER_CLEANING, METAL_POLISHING, STITCHING_REPAIR, OTHER |
| ReservationStatus | REQUESTED, CANCELLED |
